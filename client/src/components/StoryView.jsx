...
const StoryView = ({ viewStory, setViewStory }) => {
  ...
  const renderContent = () => {
    switch (viewStory.media_type) {
      ...
      case 'text':
        return (<div className='text-white w-full items-center justify-center flex text-center h-full text-2xl sm:text-4xl font-medium p-8' >{viewStory.content}</div>);
      default:
        return null;
    }
  }
  ...
  return (
    <div className='fixed inset-0 h-screen bg-black bg-opacity-90 z-110 flex items-center justify-center' style={{ backgroundColor: viewStory.media_type === 'text' ? viewStory.background_color : '#000000' }}>
      ...
      {/* close button */}
      <button onClick={handleClose} className='absolute top-4 right-4 text-white outline-none cursor-pointer text-3xl font-bold'>
        <X className='w-8 hover:scale-110 transition cursor-pointer' />
      </button>
