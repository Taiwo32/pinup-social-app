...
const ChatBox = () => {
  ...
  return user && (
    <div className='flex flex-col h-screen'>
            ...
            <div className='space-y-4 max-w-4xl mx-auto'> 
                {
                    messages.toSorted((a,b)=> new Date(a.createdAt) - new Date(b.createdAt)).map((message,index)=>(
                        <div key={index} className={`flex flex-col ${message.to_user_id !== user._id ? 'items-start' : 'items-end'}`}> 
                            <div className={`p-2 text-sm  bg-white text-slate-700 max-w-sm rounded-lg shadow ${message.to_user_id !== user._id ? 'rounded-bl-none' : 'rounded-br-none'}`}>
        ...
        <div className='px-4'>
            <div className='flex items-center gap-3 pl-5 bg-white w-full max-w-xl mx-auto border border-gray-300 shadow rounded-full mb-5'>
                ...
                <label htmlFor="image">
                    {
                        images ? <img src={URL.createObjectURL(images)} alt="" className='size-8 rounded' />
                        : <ImageIcon className='size-7 text-gray-400  cursor-pointer'/>
                    }
                    <input type="file" id='image' accept='image/*' hidden onChange={(e)=> setImages(e.target.files[0])} />
                </label>

                <button onClick={sendMessage} className='bg-linear-to-br from-indigo-500 to-purple-600 hover:from-indigo-700 hover:to-purple-800 active:scale-95 rounded-full text-white cursor-pointer'>
